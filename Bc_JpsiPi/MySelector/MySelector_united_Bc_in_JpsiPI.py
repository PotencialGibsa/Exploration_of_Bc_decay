...
def tree_fill(my_vars, chain):
	...
	for evt in range(nevt):
	    ...
	    for cand in range(_nCand):
	        ...
	        genM[0] = (chain.gen_pika1_p4 + chain.gen_muonN_p4 + chain.gen_muonP_p4).M();KA1_drTRG[0] = chain.KA1_drTRG[cand];KA1_dptTRG[0] = chain.KA1_dptTRG[cand]
...
def main():
	...
	if len(MyFileNames) == 0:
		print("No files to proccess")
		raise SystemExit
	#
	#making chain
	chain = chain_make(MyFileNames)
	#
	#file out defining and checking the existense of such a files
	_fileOUT = '/eos/home-d/dshmygol/SimpleFile_MC_Bc_in_JpsiPI_v6_trigger_matching.root'

	fileOUT = fileOUT_check(_fileOUT)
	#
