...
Xb_frame::Xb_frame(const edm::ParameterSet& iConfig) :
    ...
    PI1_ips(0), 	PI1_drTRG(0), PI1_dptTRG(0),
...
Xb_frame::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	gen_pika1_p4  .SetPtEtaPhiM(0.,0.,0.,0.);
	//gen_kaon1_p4  .SetPtEtaPhiM(0.,0.,0.,0.);
	//cout << "Start GEN info "<< event << endl;
	...
	if ( GenParticles.isValid() ) {
		for ( reco::GenParticleCollection::const_iterator itParticle = GenParticles->begin(); itParticle != GenParticles->end(); ++itParticle )
		{
			...
			if (pdgId == 541) {
				//cout << "pdgId: " << pdgId << "; daughter 0: " << abs(itParticle->daughter(0)->pdgId()) << " ; daughter 1: " << abs(itParticle->daughter(1)->pdgId()) << endl;
   				//if (itParticle->numberOfDaughters() < 2) continue;
   				if (itParticle->daughter(0) == NULL) continue;
   				if (itParticle->daughter(1) == NULL) continue;
				if ((itParticle->daughter(1)->pdgId() == 211) && (itParticle->daughter(0)->pdgId() == 443)) {
					decatype = 0; //
				}
				if ((itParticle->daughter(1)->pdgId() == 321) && (itParticle->daughter(0)->pdgId() == 443)) {
					decatype = 1; //
				   //cout << " decatype: " << decatype << endl;
				}
				if ((itParticle->daughter(1)->pdgId() == 213) && (itParticle->daughter(0)->pdgId() == 443)) {
					decatype = 2; //
				}
				...
				if (n < 2) continue;
				if (bc->daughter(0) == NULL) continue;
				if (bc->daughter(1) == NULL) continue;
				if (decatype == 0) {
					const reco::Candidate* jps = bc->daughter(0);
					const reco::Candidate* pi1 = bc->daughter(1);

					gen_pika1_p4.SetPtEtaPhiM(pi1->pt(),pi1->eta(),pi1->phi(),pi1->mass());
				...
				if (decatype == 1) {
					const reco::Candidate* jps = bc->daughter(0);
					const reco::Candidate* ka1 = bc->daughter(1);

					gen_pika1_p4.SetPtEtaPhiM(ka1->pt(),ka1->eta(),ka1->phi(),ka1->mass());
				...
				if (decatype == 2) {
					const reco::Candidate* jps = bc->daughter(0);
					const reco::Candidate* rho = bc->daughter(0);
					//rho -> pi+ pi0
					const reco::Candidate* pi1 = rho->daughter(0);
					if (pi1 == NULL) continue;
					if (pi1->pdgId() != 211)  continue;
					gen_pika1_p4.SetPtEtaPhiM(pi1->pt(),pi1->eta(),pi1->phi(),pi1->mass());
	...
	for (size_t i = 0; i < TriggerFilters_.size(); i++)
	{
	    ...
	    { // this tells us if the filter has been fired
	        ...
	        for (size_t i = 0; i < nObjects; i++)
	        {
	            ...
	            if (abs(tObject.id())==211) //// trigger code assigns PID, in our case 321 = kaon
	            {
...
void Xb_frame::beginJob()
{
	...
	wwtree->Branch("gen_pika1_p4"       , "TLorentzVector", &gen_pika1_p4  );
