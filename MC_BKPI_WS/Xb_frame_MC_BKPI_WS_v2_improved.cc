	...
	decatype(0)
{
	...
	tok_v0_     = consumes<reco::VertexCompositeCandidateCollection>(   edm::InputTag("generalV0Candidates:Kshort"));
...
Xb_frame::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	if ( GenParticles.isValid() ) {
		for ( reco::GenParticleCollection::const_iterator itParticle = GenParticles->begin(); itParticle != GenParticles->end(); ++itParticle )
		{
   			...
   			if (abs(pdgId) == 541) {
   				if (itParticle->numberOfDaughters() < 2) continue;
				if (abs(itParticle->daughter(0)->pdgId()) == 521){
					decatype = 2; //
				}
				if (abs(itParticle->daughter(0)->pdgId()) == 523){
					decatype = 3; //
				}
				if (abs(itParticle->daughter(1)->pdgId()) == 521){
					decatype = 0; //
				}
				if (abs(itParticle->daughter(1)->pdgId()) == 523){
					decatype = 1; //
				}
				if (abs(itParticle->daughter(0)->pdgId()) == 10533){
					decatype = 6; //
				}
				if (abs(itParticle->daughter(0)->pdgId()) == 535 && abs(itParticle->daughter(0)->daughter(0)->pdgId()) == 521){
					decatype = 5; //
				}
				if (abs(itParticle->daughter(0)->pdgId()) == 535 && abs(itParticle->daughter(0)->daughter(0)->pdgId()) == 523){
					decatype = 4; //
				}
				const reco::Candidate* bc = &(*itParticle);

				int n = itParticle->numberOfDaughters();
				if (n < 2) continue;

   				if ((decatype == 2) || (decatype == 3) || (decatype == 4) || (decatype == 5) || (decatype == 6)) {
       				const reco::Candidate* pi1 = bc->daughter(1);

       				if (pi1 == NULL) continue;
       				if (abs(pi1->pdgId()) != 211)      continue;

       				gen_pion1_p4.SetPtEtaPhiM(pi1->pt(),pi1->eta(),pi1->phi(),pi1->mass());

       				if((decatype == 2) || (decatype == 3)) {

           				const reco::Candidate* ka1 = bc->daughter(2);

						if (ka1 == NULL) continue;
						if (abs(ka1->pdgId()) != 321)      continue;

						gen_kaon1_p4.SetPtEtaPhiM(ka1->pt(),ka1->eta(),ka1->phi(),ka1->mass());

           				if (decatype == 2) {
               				const reco::Candidate* bu = bc->daughter(0);

               				if (bu == NULL) continue;
               				if (abs(bu->pdgId()) != 521)      continue;

               				n = bu->numberOfDaughters();
               				if (n < 2)   continue;

               				const reco::Candidate* jps = bu->daughter(0);
               				const reco::Candidate* ka2 = bu->daughter(1);

							if (jps == NULL) continue;
							if (ka2 == NULL) continue;
							if (abs(jps->pdgId()) != 443)      continue;
							if (abs(ka2->pdgId()) != 321)      continue;

							n = jps->numberOfDaughters();
							if (n < 2)   continue;

							const reco::Candidate* mu1 = jps->daughter(0);
							const reco::Candidate* mu2 = jps->daughter(1);

							if (mu1 == NULL) continue;
							if (mu2 == NULL) continue;
							if (abs(mu1->pdgId()) != 13)       continue;
							if (abs(mu2->pdgId()) != 13)       continue;

							if (mu1->charge() > 0)    {
                   				gen_muonP_p4.SetPtEtaPhiM(mu1->pt(),mu1->eta(),mu1->phi(),mu1->mass());
                   				gen_muonN_p4.SetPtEtaPhiM(mu2->pt(),mu2->eta(),mu2->phi(),mu2->mass());
               				}
               				else {
                   				gen_muonP_p4.SetPtEtaPhiM(mu2->pt(),mu2->eta(),mu2->phi(),mu2->mass());
                   				gen_muonN_p4.SetPtEtaPhiM(mu1->pt(),mu1->eta(),mu1->phi(),mu1->mass());
               				}

               				gen_kaon2_p4.SetPtEtaPhiM(ka2->pt(),ka2->eta(),ka2->phi(),ka2->mass());
           				}//2dec

           				if (decatype == 3) {

							const reco::Candidate* bust = bc->daughter(0);

							if (bust == NULL) continue;
							if (abs(bust->pdgId()) != 523)     continue;

							n = bust->numberOfDaughters();
							if (n < 2)   continue;

							const reco::Candidate* bu = bust->daughter(0);
       				...
       				if ((decatype == 4) || (decatype == 5)) {

						const reco::Candidate* bs2 = bc->daughter(0);

						if (bs2 == NULL) continue;
						if (abs(bs2->pdgId()) != 535)      continue;

						n = bs2->numberOfDaughters();
						if (n < 2) continue;

						const reco::Candidate* ka1 = bs2->daughter(1);
						...
						if (decatype == 4) {
               				...
               				const reco::Candidate* bust = bs2->daughter(0);
           				...
           				if (decatype == 5) {
							...
							const reco::Candidate* bu = bs2->daughter(0);
					...
					if (decatype == 6) {
						...
						const reco::Candidate* bs1 = bc->daughter(0);

						if (bs1 == NULL) continue;
						if (abs(bs1->pdgId()) != 10533)    continue;

						n = bs1->numberOfDaughters();
						if (n < 2) continue;

						const reco::Candidate* bust = bs1->daughter(0);
						const reco::Candidate* ka1  = bs1->daughter(1);
   				...
   				if ((decatype == 0) || (decatype == 1)) {

					const reco::Candidate* kst0 = bc->daughter(0);

					if (kst0 == NULL) continue;
					if (abs(kst0->pdgId()) != 313)     continue;

					n = kst0->numberOfDaughters();
					if (n < 2)                         continue;

					const reco::Candidate* ka1 = kst0->daughter(0);
					const reco::Candidate* pi1 = kst0->daughter(1);
					...
					gen_pion1_p4.SetPtEtaPhiM(pi1->pt(),pi1->eta(),pi1->phi(),pi1->mass());
	   				...
	   				if (decatype == 0) {
						...
						const reco::Candidate* bu   = bc->daughter(1);
       				...
       				if (decatype == 1) {
	           			...
	           			const reco::Candidate* bust = bc->daughter(1);
	...
	//cout << "finished  GEN info "<< event << " decatype" << decatype << std::endl;
	...
	if (2<1)  ///  2016 !!! FOR MC DO MANUALLY
	{
	    /////  2016 !!!
	        ...
	        for (vector < pat::GenericParticle >::const_iterator iTrack1 = thePATTrackHandle->begin(); iTrack1 != thePATTrackHandle->end(); ++iTrack1)
	        {
	            ...
	            for (vector < pat::GenericParticle >::const_iterator iTrack2 = thePATTrackHandle->begin(); iTrack2 != thePATTrackHandle->end(); ++iTrack2)
	            {
	                ...
	                for (vector < pat::GenericParticle >::const_iterator iTrack3 = thePATTrackHandle->begin(); iTrack3 != thePATTrackHandle->end(); ++iTrack3)
	                {
	                    ...
	                    KinematicParticleVertexFitter dFitter;
	                    fitgood = 1;
	                    try
	                    {
...
void Xb_frame::beginJob()
{
	...
	wwtree->Branch("decatype"           , &decatype         , "decatype/I"  );
	...
	wwtree->Branch("gen_muonP_p4"       , "TLorentzVector", &gen_muonP_p4  );
	wwtree->Branch("gen_muonN_p4"       , "TLorentzVector", &gen_muonN_p4  );
	wwtree->Branch("gen_pion1_p4"       , "TLorentzVector", &gen_pion1_p4  );
	wwtree->Branch("gen_kaon1_p4"       , "TLorentzVector", &gen_kaon1_p4  );
	wwtree->Branch("gen_kaon2_p4"       , "TLorentzVector", &gen_kaon2_p4  );
}
