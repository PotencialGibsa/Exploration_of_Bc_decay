...
MyFileNames = glob.glob('/eos/home-d/dshmygol/Bc_Bfinder/201?_MC_Bc_in_BKPI_WS_v6_trigger_matching/*/*/*/000?/BFinder*.root')
...
for name in MyFileNames:
    ...
    chain.Add(name)
    Name_count+=1
_fileOUT = '/eos/home-d/dshmygol/SimpleFile_MC_Bc_in_BKPI_WS_v7_trigger_matching.root'
...
my_vars = [
    ...
    'mu2_pt', 'mu2_CV_pt',
    #gen
    'gen_muonP_delta', 'gen_muonN_delta', 'gen_kaon1_delta', 'gen_kaon2_delta',
    'gen_pion1_delta',
...
K1P4, K1P4_CV, K2P4, K2P4_CV, PI3P4, PI3P4_CV, MU1P4, MU2P4, MU1P4_CV, MU2P4_CV, JPSIP4, BP4, BcP4, BandKP4, BandPIP4, PIandKP4, BP4CV = [TLorentzVector() for i in range(17)]
...
for evt in range(nevt):
	...
	for cand in range(_nCand):
		...
		if chain.decatype == 1 or chain.decatype == 0:
			K2P4.SetXYZM (chain.PI3_px[cand], chain.PI3_py[cand], chain.PI3_pz[cand], PDG_KAON_MASS)
			K2P4_CV.SetXYZM (chain.PI3_px_CV[cand], chain.PI3_py_CV[cand], chain.PI3_pz_CV[cand], PDG_KAON_MASS)
			K2_pt[0] = K2P4.Pt()
			K2CV_pt[0] = K2P4_CV.Pt()
			K2_ips[0] = chain.PI3_ips[cand]
			#K2_chrg[0] = chain.PI3_ch[cand]
			K2_chrg[0] = chain.K2_ch[cand]
			# assigning Pion3 info
			PI3P4.SetXYZM (chain.K2_px[cand], chain.K2_py[cand], chain.K2_pz[cand], PDG_PION_MASS)
			PI3P4_CV.SetXYZM (chain.K2_px_CV[cand], chain.K2_py_CV[cand], chain.K2_pz_CV[cand], PDG_PION_MASS)
			PI3_pt[0] = PI3P4.Pt()
			PI3CV_pt[0] = PI3P4_CV.Pt()
			PI3_ips[0] = chain.K2_ips[cand]
			PI3_chrg[0] = chain.K2_ch[cand]
		...
		####Special
		BandKP4 = BP4CV + K2P4_CV
		BandK_mass[0]= BandKP4.M() - BP4CV.M() + PDG_B_MASS
		BandPIP4 = BP4CV + PI3P4_CV
		BandPI_mass[0]= BandPIP4.M() - BP4CV.M() + PDG_B_MASS
		PIandKP4 = K2P4_CV + PI3P4_CV
		...
		Bc_mass[0]= (BP4CV + K2P4_CV + PI3P4_CV).M() - BP4CV.M() + PDG_B_MASS
		Bc_mas1[0]= chain.BK_mass[cand]
		...
		B_Bccos3[0] = DirectionCos3 ( BV - BcV, BP3)
		# pereputaem K2P4 and PI3P4 in DeltaR for genmathching correct because wrong sign
		gen_muonP_delta[0] = chain.gen_muonP_p4.DeltaR(MU1P4_CV)
		gen_muonN_delta[0] = chain.gen_muonN_p4.DeltaR(MU2P4_CV)
		gen_kaon1_delta[0] = chain.gen_kaon2_p4.DeltaR(K1P4)
		gen_kaon2_delta[0] = chain.gen_kaon1_p4.DeltaR(PI3P4)
		gen_pion1_delta[0] = chain.gen_pion1_p4.DeltaR(K2P4)
